...
const FornecedoresCRUD = () => {
    ...
    try {
      if (editing) {
        const { error } = await supabase.from("fornecedores").update({
          name: form.name, cnpj: form.cnpj, contact: form.contact || null,
        }).eq("id", editing.id);
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      toast.error(err.message?.includes("violates") ? "Não é possível excluir: fornecedor possui movimentações" : err.message);
    }
  };
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        <Dialog open={dialogOpen} onOpenChange={(open) => { setDialogOpen(open); if (!open) resetForm(); }}>
          <DialogTrigger asChild>
            <Button size="sm"><Plus className="w-4 h-4 mr-2" />Novo Fornecedor</Button>
          </DialogTrigger>
          <DialogContent>
            <DialogHeader>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                <Label>Contato</Label>
                <Input value={form.contact} onChange={e => setForm({ ...form, contact: e.target.value })} placeholder="Telefone ou e-mail" />
              </div>
      ...
      <div className="bg-card rounded-xl border border-border overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
                      ...
                      <AlertDialog>
                        ...
                        <AlertDialogContent>
                          <AlertDialogHeader>
                            ...
                            <AlertDialogDescription>Só é possível excluir fornecedores sem movimentações vinculadas.</AlertDialogDescription>
                          </AlertDialogHeader>
                          <AlertDialogFooter>
            ...
            {filtered.length === 0 && (
              <tr><td colSpan={5} className="p-8 text-center text-muted-foreground">Nenhum fornecedor encontrado</td></tr>
            )}
          </tbody>
        </table>
      </div>
